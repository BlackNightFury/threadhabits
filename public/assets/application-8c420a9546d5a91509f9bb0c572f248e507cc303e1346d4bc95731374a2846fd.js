...
(function() {
  $(function() {
    $("#toggle-filter-menu-desktop").click(function() {
      return $("#filter-panel").toggle();
    });
    return $("#navbar-search-xs").click(function(e) {
      e.preventDefault();
      return $("#navbar-search-form-xs").slideToggle();
    });
  });
...
(function() {
  this.Follow = {
    initializeFollowings: function() {
      return $("#follow-person").on("click", function(e) {
        var $elem;
        e.preventDefault();
        $elem = $(this);
        ...
        return $.ajax({
          ...
          success: function(response) {
            ...
            return $(".profile-desc .followers span.value").text(response.followers);
          },
  ...
  this.Inbox = {
    validateForm: function() {
      ...
      return $('#messageFormInbox').validate({
        rules: {
          "message[body]": 'required'
        },
        submitHandler: function(form) {
    ...
    submitMessage: function(form) {
      var $form, $modal_body, action, method, that;
      that = this;
      $form = $(form);
      ...
      return $.ajax({
        ...
        complete: function() {
          ...
          return $modal_body.find(".message-body-form").hide();
        },
        success: function(response) {
          if (response.status === 200) {
            $modal_body.find(".message-body-success").show();
            return $modal_body.find(".inbox-link").attr("href", response.inbox_path);
          } else {
            $modal_body.find(".message-body-success").hide();
            return $modal_body.find(".message-body-failure").show();
          }
        },
        error: function(response) {
          $modal_body.find(".message-body-success").hide();
    ...
    initializeMessageBox: function() {
      return $("#message-box-link").click(function(e) {
        e.preventDefault();
        return $.get($(this).data("url"), function(response) {
          console.log("Hello");
          $("#messageModal").modal({
            show: true,
            backdrop: "static"
          });
          return $("#messageModal").on('shown.bs.modal', function() {
            Inbox.validateForm();
            return Inbox.hideMessageBox();
          });
        });
      });
    },
    hideMessageBox: function() {
      return $("#messageModal").on('hidden.bs.modal', function() {
        return $(".message-box").empty();
      });
    }
  };
  ...
  this.Profiles = {
    validateForm: function() {
      return $("form#editProfilesForm").validate({
        rules: {
  ...
  this.Registrations = {
    validateForm: function() {
      var remoteMethod, remoteUrl;
      remoteUrl = "/people.json?";
      remoteMethod = "POST";
      return $('#new_person').validate({
        rules: {
          "person[first_name]": 'required',
          "person[last_name]": 'required',
          "person[username]": {
  ...
  this.Listings = {
    redirect_to: "",
    validateForm: function() {
      var remoteMethod, remoteUrl, that;
      that = this;
      remoteUrl = $('form.listing-form').attr("action") + ".json?";
      remoteMethod = $('form.listing-form').data("new-record") ? "POST" : "PUT";
      return $('form#listingForm').validate({
        debug: true,
        errorClass: "error",
        rules: {
          "listing[name]": {
    ...
    submitListings: function(form) {
      var $form, $upload, action, method, that;
      that = this;
      $form = $(form);
        ...
        success: function(response) {
          ...
          if (response.status === 200) {
            ...
            if ($form.data("new-record") === false && totalFiles === 0) {
              return window.location.href = that.redirect_to;
            } else {
              return $upload.click();
            }
          } else {
            $form.find("input[type='submit']").prop("disabled", false);
            return console.log("Couldn't Save Listing at the moment");
          }
        },
        error: function(response) {
          console.log("error");
    ...
    updateNumberOfFilesText: function() {
      var frames, text;
      frames = $(".listing-uploads .file-preview-frame").length;
      text = frames + " " + (frames > 1 ? "files" : "file");
      return $(".file-caption-name").html('<i class="glyphicon glyphicon-file kv-caption-icon"></i>' + text + " selected");
    },
    toggleFileUploadValidation: function() {
      return $(document).on("click", ".listing-uploads .fileinput-remove, .listing-uploads .kv-file-remove", function() {
        return setTimeout(function() {
          var totalFiles;
          totalFiles = $("#listing_upload_photos").fileinput('getFilesCount');
          if (totalFiles === 0) {
            return $("#listing_upload_photos").rules('add', {
              required: true
            });
          }
        }, 1000);
    ...
    refreshFileInput: function(imgsPath, imgsConfig) {
      var minFileCount, parsedConfig, that;
      if (imgsPath == null) {
        imgsPath = [];
      }
      if (imgsConfig == null) {
      ...
      that = this;
      parsedConfig = [];
      imgsConfig.forEach(function(val, index) {
        var parsed;
        console.log(index);
        parsed = JSON.parse(val);
        parsedConfig.push(parsed);
        return setTimeout(function() {
          var removeElems, uploadId;
            ...
            if (removeElems.length > 1) {
              ...
              return $.ajax({
                ...
                success: function(result) {
                  ...
                  elem.closest(".file-preview-frame").fadeOut(500, function() {
                    elem.closest(".file-preview-frame").remove();
                    return that.updateNumberOfFilesText();
                  });
                },
                error: function() {
              ...
              return window.setTimeout(function() {
                $('#file-upload-error-container').fadeOut("slow", function() {
                  return $('#file-upload-error-container ul').remove();
                });
                return elem.prop("disabled", false);
              }, 2000);
            }
      ...
      return $("#listing_upload_photos").fileinput('refresh', {
    ...
    infiniteScroll: function() {
      if ($('#infinite-scrolling').size() > 0) {
        $(window).on('scroll', function() {
          var more_listings_url, more_listings_url_fix;
    ...
    initializeListingsForm: function() {
      var that;
      that = this;
      that.validateForm();
      $("#listing_company_id").select2();
      $(".product-type-listing-input").on("change", function(e) {
        return $.get("/listings/collect_size/" + ($(this).val()) + ".js", function(data) {
          that.validateForm();
          return console.log("Sizes Collection Completed");
        }).fail(function() {
          alert("Couldn't fetch sizes at the moment");
        });
      });
      setTimeout(function() {
        return $(".listing-uploads .fileinput-remove").remove();
      }, 100);
      $(document).on("click", ".listing-uploads .kv-file-remove", function() {
        return setTimeout(function() {
          return that.updateNumberOfFilesText();
        }, 1000);
      });
      return $("#listing_upload_photos").fileinput({
        ...
        setTimeout(function() {
          console.log(event);
          return this.location.href = that.redirect_to;
        }, 3000);
      }).on('filebatchuploaderror', function(event, data) {
        return $("form").find("input[type='submit']").prop("disabled", false);
      }).on('fileerror', function(event, data) {
        $("form").find("input[type='submit']").prop("disabled", false);
        return $("form").find("input[type='submit']").val("Submit");
      });
    }
  };
  ...
  this.Home = {
    initializeFilters: function() {
      var nodes, nodesInput, nonLinearSlider;
      $.fn.followTo = function(pos) {
        var $this, $window, navBottomHeaderHeight, navTopHeaderHeight, topOffsetFilter;
        $this = this;
        $window = $(window);
      ...
      nodes = [document.getElementById('lower-price-span'), document.getElementById('upper-price-span')];
      nodesInput = [document.getElementById('lower-price-input'), document.getElementById('upper-price-input')];
      noUiSlider.create(nonLinearSlider, {
        connect: true,
        behaviour: 'tap',
        start: [nodesInput[0].value, nodesInput[1].value],
        range: {
          'min': [0],
          'max': [15000]
        }
      });
      nonLinearSlider.noUiSlider.on('end', function(e) {
        console.log("end");
        return $("form").submit();
      });
      ...
      $('.collapse-filter-link').on('click', function(e) {
        var target;
      ...
      if (Settings.getUrlParameter("filters[product_type]") === "") {
        return $(".listing-filters .in").each(function() {
          var target;
          target = $(this).prev().find("a span");
          if (target.html() === "-") {
            target.html("+");
            return target.parent().addClass("active-strip");
          } else {
            return target.html("-");
          }
        });
...
(function() {
  ...
  this.GoogleMaps = (function() {
    function GoogleMaps() {
      this.initAutocomplete = bind(this.initAutocomplete, this);
      this.holdFormOnAutocomplete = bind(this.holdFormOnAutocomplete, this);
      this.autocompleteDropDownCSS = bind(this.autocompleteDropDownCSS, this);
      this.getPlaceDetails = bind(this.getPlaceDetails, this);
      this.setMarkersMap = bind(this.setMarkersMap, this);
      this.placeMarkers = bind(this.placeMarkers, this);
      this.markers = [];
      this.ready = void 0;
    }

    GoogleMaps.initMap = function(mapContainer, opt, callback) {
      var location, map, markerOpt, self;
      if (opt == null) {
        opt = {};
      }
      markerOpt = {};
    ...
    GoogleMaps.prototype.placeMarkers = function(map, markerOpt, location) {
      var infowindow, marker, title;
    ...
    GoogleMaps.prototype.setMarkersMap = function(map) {
      var i, results;
      i = 0;
      results = [];
    ...
    GoogleMaps.prototype.getPlaceDetails = function(map, place_id, callback) {
      var result, service;
      result = {};
    ...
    GoogleMaps.prototype.autocompleteDropDownCSS = function() {
      return setTimeout(function() {
        if ($(window).width() < 667) {
          $(".pac-container").show();
          $(".pac-container").css("top", $(".google-autocomplete").offset().top + 5 + "px");
        }
        return $(".pac-container").css("z-index", "9999");
      }, 5);
    };

    GoogleMaps.prototype.holdFormOnAutocomplete = function() {
      var input, that;
      that = this;
      ...
      $(input).focus(function() {
        ...
        return that.autocompleteDropDownCSS();
      });
      $(input).blur(function() {
        return $(this).closest("form").find("input[type='submit'], button[type='submit']").prop("disabled", false);
      });
      return $(input).keypress(function() {
        return that.autocompleteDropDownCSS();
      });
    };

    GoogleMaps.prototype.initAutocomplete = function(map, input, callback) {
      var autocomplete, infowindow, marker;
      $(input).addClass("google-autocomplete");
      this.holdFormOnAutocomplete();
      ...
      autocomplete.addListener('place_changed', function() {
        var address, location_name, place;
        infowindow.close();
        ...
        if (place.address_components) {
          ...
          location_name = [place.address_components[3] && place.address_components[3].short_name || 0, place.address_components[4] && place.address_components[4].short_name || 0];
        }
        infowindow.setContent('<div><strong>' + place.name + '</strong><br>' + address);
        infowindow.open(map, marker);
        if (location_name[0] === 0 || location_name[1] === 0) {
...
(function() {
  ...
  Native = (function() {
    var disabledEventPropagation, toggleSideMenu, topSearchBar;

    function Native() {}

    Native.prototype.filtersTweaks = function() {
      toggleSideMenu("#toggle-filter-menu", "#filter-menu");
      return $("ul.sub-categories li a").each(function() {
        if ($(this).text().toLowerCase() === Settings.getUrlParameter("filters[product_type]")) {
          return $(this).parent().addClass("active-category");
        }
      });
    };

    Native.prototype.headerTweaks = function() {
      topSearchBar();
      return toggleSideMenu(".navbar-toggle.collapsed", "#navbar-menu");
    };
    ...
    toggleSideMenu = function(elem, colapsedElem) {
      $(elem).click(function(e) {
        disabledEventPropagation(e);
        $(colapsedElem).addClass("toggle-side-menu");
        $('.main-container .banner-row').hide();
        return setTimeout(function() {
          return $(".main-container").addClass("main-toggle-animation");
        }, 200);
      });
      return $(".dismiss-times").click(function() {
        $(colapsedElem).removeClass("toggle-side-menu");
        $('.main-container .banner-row').show();
        return setTimeout(function() {
          return $(".main-container").removeClass("main-toggle-animation");
        }, 200);
      });
    };
    ...
    topSearchBar = function() {
      $(".navbar-search").click(function(e) {
        ...
        return $(".navbar-search-form").css("left", "0px");
      });
      $(".navbar-search-form input").click(function(e) {
        return disabledEventPropagation(e);
      });
      $(".navbar-search-form .submit-search").click(function(e) {
        disabledEventPropagation(e);
        return $(this).closest("form").submit();
      });
      return $(".cancel-search").click(function() {
        $(".navbar-header").css("left", "0px");
        return $(".navbar-search-form").css("left", "1000px");
      });
    };
  ...
  this.Mobile = new Native;

}).call(this);
(function() {
  var previewImageElem;

  previewImageElem = function(options) {
    var img;
    if (options == null) {
      options = {};
    }
  ...
  window.clearPreviewImage = function(elem) {
    var parent;
  ...
  window.createPreviewImage = function(elem, options) {
    var file, img, reader;
    if (options == null) {
      options = {};
    }
  ...
  $(document).on("turbolinks:load", function() {
    var closebtn;
    $('.profile .listing-tabs a').click(function(e) {
      e.preventDefault();
    ...
    return $.each($(".image-preview"), function() {
      var content, img;
...
(function() {
  ...
  window.MainSubject = (function() {
    function MainSubject() {}

    MainSubject.observers = [];

    MainSubject.subscribe = function(observer) {
      return this.observers.push(observer);
    };

    MainSubject.unsubscribe = function(fn) {
      return this.observers = this.observers.filter(function(item) {
        if (item === fn) {
          return item;
        }
      });
    ...
    MainSubject.publish = function(message, key, value) {
      return this.observers.forEach(function(item) {
        if (Array.isArray(message)) {
          return message.forEach(function(msg) {
            return item[msg](key, value);
          });
        } else {
          return item[message](key, value);
        }
      });
    };
  ...
  window.Settings = (function() {
    function Settings() {
      this.updateLocationInputs = bind(this.updateLocationInputs, this);
      this.updateCurrentLocation = bind(this.updateCurrentLocation, this);
      this.showPosition = bind(this.showPosition, this);
      this.initializeLocation = bind(this.initializeLocation, this);
    }

    Settings.current_location = {};

    Settings.prototype.initializeLocation = function() {
    ...
    Settings.prototype.showPosition = function(position) {
      if (gon && gon.current_location !== void 0) {
    ...
    Settings.prototype.updateCurrentLocation = function(key, value) {
      return Settings.current_location[key] = value;
    };

    Settings.prototype.updateLocationInputs = function(key, value) {
      if (this.inputs[key]) {
        return this.inputs[key].value = value;
      }
    };

    Settings.clearTurbolinksCache = function() {
      return document.addEventListener('turbolinks:before-visit', function() {
    ...
    Settings.getUrlParameter = function(sParam, parametersType) {
      var i, objectParameter, sPageURL, sParameterName, sURLVariables;
      if (parametersType == null) {
        parametersType = "plain";
      }
      sPageURL = decodeURIComponent(window.location.search.substring(1));
      sURLVariables = parametersType === "object" ? sPageURL.split('%5B') : sPageURL.split('&');
      i = 0;
      while (i < sURLVariables.length) {
        objectParameter = sURLVariables[i];
        sParameterName = parametersType === "object" ? objectParameter.split('%5D') : objectParameter.split('=');
  ...
  window.Location = (function(superClass) {
    extend(Location, superClass);

    function Location(map, presetLocation) {
      if (map == null) {
        map = true;
      }
      ...
      this.startMap = bind(this.startMap, this);
      this.map = map;
      ...
      this.container = document.getElementById("google-maps");
      if (this.map === false) {
        $(this.container).css("height", "0px");
      }
    }

    Location.prototype.startMap = function(callback) {
      var mapInterval;
      if (gon && gon.current_location !== void 0) {
        Settings.current_location = gon.current_location;
      }
      return mapInterval = setInterval(function() {
    ...
    Location.prototype.setMap = function() {
      var that;
      that = this;
      return this.startMap(function() {
        return GoogleMaps.initMap(that.container, Settings.current_location);
      });
    };

    Location.prototype.setMapForm = function() {
      var that;
      that = this;
      return this.startMap(function() {
        var googleMap, map;
        ...
        map = GoogleMaps.initMap(that.container, Settings.current_location, function(marker) {
          ...
          if (marker.placeId) {
            return googleMap.getPlaceDetails(map, marker.placeId, function(place) {
              MainSubject.publish(["updateCurrentLocation", "updateLocationInputs"], "location", place.formatted_address);
              return MainSubject.publish(["updateCurrentLocation", "updateLocationInputs"], "place_id", marker.placeId);
            });
          }
        });
        googleMap.initAutocomplete(map, that.inputs.location, function(place) {
          ...
          return MainSubject.publish(["updateCurrentLocation", "updateLocationInputs"], "place_id", place.place_id);
        });
        return MainSubject.unsubscribe(that);
      });
    };
...
(function() {
  ...
  $(document).on("turbolinks:load", function() {
    ...
    $("li.navbar-profile-icon a.dropdown-toggle").click(function(e) {
      Turbolinks.visit("/profiles/" + ($(this).find('.profile-name').text()));
      return false;
    });
    $("li.navbar-profile-icon .dropdown-menu").on("click", function(e) {
      var target;
      target = $(e.target);
      if (!target.is('a')) {
    ...
    return $("li.navbar-profile-icon").hover((function() {
      var $this;
      $this = $(this);
      $this.toggleClass('open');
      $.get("/verify_unread_message", function(response) {
        if (response.unread === false) {
          return $this.find("span.notify_message").show();
        }
      });
    }), function() {
      $(this).toggleClass('open');
  ...
  this.AboutUs = {
    initialize: function() {
      var $body, $offset, a, path, setDefaultActive;
      ...
      if (setDefaultActive) {
        $('#about_us .side-affix ul li.active span').css("display", "inline-block");
      }
      ...
      $('#side-about-scrollspy').on('activate.bs.scrollspy', function() {
        $('#about_us .side-affix ul li').find("span").hide();
        return $('#about_us .side-affix ul li.active').find("span").css("display", "inline-block");
      });
      return $("#about_us .side-affix a").click(function() {
        var target;
